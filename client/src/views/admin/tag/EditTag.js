...
import {
 ...
 class EditTag extends Component {
        ...
        axios({
            ...
            url: `${process.env.REACT_APP_API_URL}/tag/${tagid}`,
                ...
                if(response.data.status === 'success'){ 
                  ...
                  self.setState({id:response.data.data.id,slug:response.data.data.slug,title:response.data.data.title,content:response.data.data.content})
                } else{
    ...
    render() {
        const {title,content,slug,redirectToList} = this.state
        if(redirectToList){
            return <Redirect to={`/admin/tag/tag-list`} />
        }
        return (
            <>
      <CRow>       
        <CCol xs="12" md="12">
          <CCard>
            ...
            <CCardFooter>
              <CButton type="submit" size="sm" color="primary" onClick={this.clickSubmit} ><CIcon name="cil-scrubber" /> Update</CButton>&nbsp; <Link className="btn btn-danger" to="/admin/tag/tag-list" size="sm" color="danger"> Back</Link>
            </CCardFooter>
          </CCard>
          <ToastContainer/>
        </CCol>
